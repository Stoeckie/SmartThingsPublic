 ...
 *	Button Controller
 *
 *	Author: Stoeckie, based on SmartThings
 *	Date: 2015-10-23
 */
definition(
    ...
    author: "Stoeckie",
    description: "Control devices with buttons like the Aeon Labs Key Fob as Doorbell",
...
def selectButton() {
	dynamicPage(name: "selectButton", title: "First, select your Doorbell device", nextPage: "configureButton1", uninstall: configured()) {
		section {
...
def configureButton1() {
	dynamicPage(name: "configureButton1", title: "Now let's decide how to use the button",
		install: true, uninstall: configured(), getButtonSections(1))
}
...
def executeHandlers(buttonNumber, value) {
	...
	if (notifications?.toBoolean()) sendPush(textMessage ?: "Doorbell was pressed" )

	def phone = find('phone', buttonNumber, value)
	if (phone != null) sendSms(phone, textMessage ?:"Doorbell was pressed")

    def sirens = find('sirens', buttonNumber, value)
    if (sirens != null) flashLights(sirens)
}
...
private flashLights(devices) {
	def doFlash = true
	def onFor = onFor ?: 4000
	def offFor = offFor ?: 4000
	...
	if (doFlash) {
		...
		def initialActionOn = devices.collect{it.currentSwitch != "on"}
		log.debug "$initialActionOn"
        def delay = 1L
		numFlashes.times {
			log.trace "Switch on after  $delay msec. Device $devices"         
			devices.eachWithIndex {s, i ->
